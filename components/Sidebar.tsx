        ...
        <div className="h-full">
          {routes.map((rts: any, key: number) => {
            if (!rts.children) return (<div className="" key={key}>
              <li
                className={`w-full text-base mb-2 rounded-lg menu-item ${router === rts.path ? "menu-active" : ""}`}
              >
            ...
            if (rts.children) return (<div className="" key={key}>
              <li
                className={`w-full rounded-lg menu-item-nested text-base dropdown dropdown-right mb-2 ${router.split("/")[2] === rts.path.split("/")[2] ? "menu-active" : ""}`}
              >
                ...
                <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-lg w-52">
                  {rts.children.map((chil: any, cKey: number) => {
                    return <li key={cKey}>
                      <Link href={rts.path + chil.path} className={`nested-item ${rts.path + chil.path == router ? "nested-active" : ""}`}>{chil.label}</Link>
                    </li>
                  })}
                </ul>
              </li>
